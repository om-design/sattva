...
# Shared relation + pos
REL_WEIGHTED: List[str] = [
    "REL::next_to", "REL::next_to", "REL::next_to", "REL::next_to",
    "REL::above",
    "REL::below",
]
POS2D_BUCKETS: List[str] = ["POS2D::left", "POS2D::center", "POS2D::right"]
...
def _ensure_square_rel_seeds(container: SattvaContainer) -> None:
    ...
    print("Seeding 2D square/rectangle relation primitives...")

    for rel in sorted(set(REL_WEIGHTED)):
...
def _sample_square_rel(container: SattvaContainer) -> Tuple[np.ndarray, str, str, str]:
    space = container.embedding
    rng = space.rng

    rel = str(rng.choice(REL_WEIGHTED))
...
def run_square_relations_2d(
    ...
    for i in range(steps):
        v, rel, subj_shape, obj_shape = _sample_square_rel(container)
...
POLY_TYPES: List[str] = [
    # extend with your actual polygon tags
    "POLY2D::square",
    "POLY2D::rectangle",
    "POLY2D::triangle",
    "POLY2D::pentagon",
...
def _ensure_poly_rel_seeds(container: SattvaContainer) -> None:
    eng = container.engine
    space = container.embedding
    meta = container.meta

    if meta.get("poly_rel2d_seeds_installed"):
        return

    print("Seeding 2D polygon relation primitives...")

    for rel in sorted(set(REL_WEIGHTED)):
        for subj_shape in POLY_TYPES:
            for obj_shape in POLY_TYPES:
                for pos in POS2D_BUCKETS:
                    base_ids = ["OBJ2D::pair"]
                    instr_ids = [
                        rel,
    ...
    meta["poly_rel2d_seeds_installed"] = True

def _sample_poly_rel(container: SattvaContainer) -> Tuple[np.ndarray, str, str, str]:
    space = container.embedding
    rng = space.rng

    rel = str(rng.choice(REL_WEIGHTED))
    subj_shape = str(rng.choice(POLY_TYPES))
    obj_shape = str(rng.choice(POLY_TYPES))
    pos = str(rng.choice(POS2D_BUCKETS))
...
def run_poly_relations_2d(
    container: SattvaContainer,
    steps: int,
    log_every: int = 1000,
    phase_name: str = "poly_relations_2d_a",
) -> None:
    eng = container.engine
    _ensure_poly_rel_seeds(container)

    start_step = container.meta.get("step", 0)
    t0 = time.time()

    for i in range(steps):
        v, rel, subj_shape, obj_shape = _sample_poly_rel(container)
        step_idx = start_step + i + 1
        input_id = (
            f"{phase_name}_step_{step_idx}_"
            f"{rel}_{subj_shape}_FIG_{obj_shape}_GRD"
    ...
    # Simple terminal beep when done
    sys.stdout.write("\a")
    sys.stdout.flush()
